function nClusters = main_som_alone(seed)
%clear all; clc;

%% Load previous database
rng(seed)
load('../database/dataSB.mat'); dataSB(:, end + 1) = 1;
load('../database/dataDB.mat'); dataDB(:, end + 1) = 2;
load('../database/dataDC.mat'); dataDC(:, end + 1) = 3;
database = [dataSB; dataDB; dataDC]; disp('Database loaded.')
%p = 0.7;
%r = round(length(dataSB) * 0.7);
%database = [dataSB(1:r, :); dataDB(1:r, :); dataDC(1:r, :)]; disp('Database loaded.')

%% Creating SOM
dim1 = 80;
dim2 = 50; disp('Creating SOM.')
...
% Matrix spaces
%figure, surf(u_matrix); % figure, contour(u_matrix, 20)
